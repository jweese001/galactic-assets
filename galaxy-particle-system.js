...
export class GalaxyParticleSystem {
   ...
   */
  generateSpiralParticle() {
    const { size, armCount, spiralTightness, diskThickness } = this.config;

    // Radius with concentration toward center
    const radius = Math.pow(Math.random(), 10) * size;
    ...
    const thickness = Math.pow(1 - (radius / size), 1.5) * diskThickness;
   ...
   */
  generateBarredSpiralParticle() {
    const { size, armCount, spiralTightness, diskThickness, coreSize } = this.config;
   ...
   */
  generateEllipticalParticle() {
    const { size, ellipticalFlatten } = this.config;

    // Spherical distribution with concentration toward center
    const radius = Math.pow(Math.random(), 4) * size * 0.8;
   ...
   */
  generateIrregularParticle() {
    const { size, diskThickness, irregularChaos } = this.config;
